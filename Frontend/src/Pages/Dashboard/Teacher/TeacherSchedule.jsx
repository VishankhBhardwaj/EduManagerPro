        ...
        <>
            {!attendanceWindow ? (
                <div className='flex flex-col p-3 gap-4 w-full h-full bg-white rounded-md shadow-2xl animate__animated animate__fadeIn '>
                    <div className="flex flex-col gap-4 p-2 bg-white rounded-lg shadow-md w-full md:p-4 hover:shadow-3xl">
                        {AllSchedules.filter((data)=> data.attendancetaken === true).map((daySchedule, index) => (
                            <div  key={index} className='w-full border-2 border-gray-200 p-3 cursor-pointer  hover:border-black transition-all duration-200 rounded-lg animate__animated animate__fadeIn animate__duration-1s' style={{ animationDelay: `${index * 0.4}s` }}>
                                <div className='flex flex-row justify-between'>
                                    <h1 className="text-xl font-semibold">{daySchedule.Day}</h1>
                                    <h2 className="text-xl font-thin">{`${daySchedule.attendancetaken}`}</h2>
                                </div>
                    ...
                    <div className="flex flex-col gap-4 p-2 bg-white rounded-lg shadow-md w-full md:p-4 hover:shadow-3xl">
                        {AllSchedules.filter((data)=> data.attendancetaken === false).map((daySchedule, index) => (
                            <div onClick={() => { setScheduleData(daySchedule); setattendanceWindow(true)}} key={index} className='w-full border-2 border-gray-200 p-3 cursor-pointer  hover:border-black transition-all duration-200 rounded-lg animate__animated animate__fadeIn animate__duration-1s' style={{ animationDelay: `${index * 0.4}s` }}>
                                <div className='flex flex-row justify-between'>
                                    <h1 className="text-xl font-semibold">{daySchedule.Day}</h1>
                                    <h2 className="text-xl font-thin">{`${daySchedule.attendancetaken}`}</h2>
                                </div>
                ...
                <Attendance setattendanceWindow={setattendanceWindow} scheduleData={scheduleData} />
                ) 
            }
{
    isOpen && (
